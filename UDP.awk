...
# Store start time
if (send_id == "1") { if (time < startTime) { startTime = time
}
...
# Update total received packets' size and store packets arrival time if (event == "r" && rec_id == "5") {
if (time > stopTime) {
stopTime = time
}
# Store received packet's size if (flow_id == "2") { #recvdSize += pkt_size recvdSize+=1
}
}
}
END {
printf("UDP throughput: %.2f packets/sec\n",recvdSize/stopTime) #printf("%i\t%i\t%.2f\t%.2f\t%.2f\n", transSize, recvdSize, startTime,
